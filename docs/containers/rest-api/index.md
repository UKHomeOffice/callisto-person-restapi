...
- Exposes [PersonEffectiveDateProperty ](#PersonEffectiveDateProperty) and [PersonEffectiveDateProperties ](#PersonEffectiveDateProperties ) resources via RESTful API (POST, GET and DELETE). 
- Publishes PersonEffectiveDateProperty and PersonEffectiveDateProperties resources via [callisto-person-people topic](#topics) following creation of new effective date properties, the removal of existing property resources or the removal of the owning person resource
...
#### [PersonEffectiveDateProperty ](#PersonEffectiveDateProperty) and [PersonEffectiveDateProperties ](#PersonEffectiveDateProperties ) resources
POST, GET and DELETE
...
| callisto-person-people | PersonEffectiveDateProperty resource | effectiveDateProperty.personId and effectiveDateProperty.personTenantId | creation of a new PersonEffectiveDateProperty. See [effective dates](#effective-dates) 
...
| personId| | | 1..1 |
| personTenantId| | | 1..1 |
| personVersionId| | Integer| 1..1 |
| property| | Property| 1..1 |
| effectiveDate| | datetime (ISO 8601)| 1..1 |

#### EffectiveDateProperties
Note that this resource cannot exist without a Person resource
...
| properties| a collection of property resources that all belong to the given version of the person| property | 1..* |
| effectiveDate| the effective date that all property instances in properties share| datetime (ISO 8601)| 1..1 |

#### Property
Note that this resource is not exposed directly via the RESTful API. Instead it will always be contained within the EffectiveDateProperties or the EffectiveDateProperty resources
...
| schema| the schema for the type that this property is a member of | uri| 1..1 |
...
In this case whilst it is correct to emit the FTEValue as a PersonEffectiveDateProperty. **However** if this is the ONLY change to the Person data there is no point emitting the complete Person record as it has not changed as of 1st May although it will on 1st October. 
...
```
PersonEffectiveDateProperty published upon receipt of the new value for FTEValue - 
...
```
{
  ...
  "property" : {
    "name": "fteValue",
    "value": 0.500,
    "schema" "https://callisto.digital.homeoffice.gov.uk/person/schemas/person?version=1.0.0"
  }
  ```
